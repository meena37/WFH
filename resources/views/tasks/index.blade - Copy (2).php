...
@else
    <a class="btn btn-sm btn-warning" >InActive</a> <a class="btn btn-sm btn-primary" href="{{ route('tasks.create') }}"> New Task</a>
@endif  
...
<div class="card-body pt-0">
	<table class="table align-middle table-row-dashed fs-6 gy-5"><thead>
        <tr>
	        ...
	        <td>{{ '8 hours' }}</td><td> {{ $loss_hour }}</td><td>{{ $nassigntime }}</td><td>{{ $available_hour }}</td><td>{{ $Entry_time }} min</td><td>{{ 'n' }}</td><td>{{ 'n' }}</td></tr></table>
	        ...
	        <td><a class="fs-6 text-gray-800 text-hover-primary" href="{{ route('tasks.edit',$task->id) }}">{{ $task->Task_Shift}}</a></td>
			 ...
			 <td>
                   ...
                   @if( $task->Start_Date == 0 )
											 ...
											 @elseif( $task->Start_Date != 0 && $task->Paused_Date != 0 && $task->Resume_Date == 0)
